...
private:
	...
	//Citanje iz 3okanalnong bmp formata bez alfa kanala
 /*static	bool  format_input(FILE *f, Layer &l, int sirina, int visina)
	{
	...
	static bool  format_input(FILE *f, Layer &l, int sirina, int visina,int transparentnost,int pozicijax,int pozicijay)
	{
		double odnos = 0;
		if (transparentnost == 0) odnos = 1;
		else { odnos = (transparentnost /(double) 255); }
		unsigned char info[54];
		fread(info, sizeof(unsigned char), 54, f); // read the 54-byte header
	...
	static void format_output(string ime, Layer &l)
	{
		...
		for (int i = 0; i < w; i++)
		{
			...
			img[(x + y * w) * 4 + 3] = (unsigned char)(alfa);
				img[(x + y * w) * 4 + 2] = (unsigned char)(r);
				img[(x + y * w) * 4 + 1] = (unsigned char)(g);
				img[(x + y * w) * 4 + 0] = (unsigned char)(b);
			}
		}
